...
def fft_convolve(audio: torch.Tensor,
    ...
    if len(ir_shape) == 4:
        batch_size_ir, n_ir_frames, ir_size, _ = ir_shape
    elif len(ir_shape) == 3:
        batch_size_ir, n_ir_frames, ir_size = ir_shape
    else:
        raise ValueError(f"Unexpected ir_shape dimensions: {ir_shape}")
...
def cross_fade_time_varying_fir(audio_fft,
    ...
    audio_frames_out = torch.fft.irfft(audio_ir_fft, signal_ndim=1, signal_sizes=(fft_size,))

    audio_ir_fft_prev = torch.view_as_real(audio_ir_fft_prev)
    audio_frames_out_prev = torch.fft.irfft(audio_ir_fft_prev, signal_ndim=1, signal_sizes=(fft_size,))
